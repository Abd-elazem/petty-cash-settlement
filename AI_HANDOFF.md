...
- 2026-07-23 — QA Stabilization Item 1 (Manager Inbox 403 in dev mode) fixed: `DevelopmentCurrentUserContext` updated to read `X-Dev-Role` and `X-Dev-Username` headers, allowing local testing of Approver role without code changes; `AuthenticationServiceCollectionExtensions` updated to register `IHttpContextAccessor` in dev mode; frontend `httpClient.ts` updated to send dev headers when `authEnabled` is false. Awaiting client `dotnet build` + `npm run build` verification.
- 2026-07-23 — QA Stabilization Item 2 (Manager Inbox empty in dev mode) fixed: `DevelopmentCurrentUserContext` email domain corrected from `@canex.local` to `@canex.com` to match seed data `AppUserProfile` entries, ensuring submitted settlements correctly appear in the Approver's inbox.
- 2026-07-23 — QA Stabilization Item 3 (Missing Reopen UI for Rejected settlements) fixed: Added `useReopenSettlement` hook, `ReopenSettlementDialog` component, and wired "Reopen Settlement" action button in `SettlementDetailPage.tsx` (visible only when status is "Rejected"). Backend endpoint was already implemented and verified. Awaiting client `npm run build` verification.
